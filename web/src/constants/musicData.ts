export interface MusicItem {
    name: string;
    level: number;
    date: string;
}

export const musicData: MusicItem[] = [
    { name: "Für Elise", level: 2, date: "02/06" },
    { name: "Moonlight Sonata", level: 3, date: "02/06" },
    { name: "Clair de Lune", level: 2, date: "02/06" },
    { name: "Prelude in C Major", level: 2, date: "02/06" },
    { name: "Gymnopédie No.1", level: 1, date: "02/06" },
    { name: "Nocturne in E-flat Major", level: 3, date: "02/06" },
    { name: "The Entertainer", level: 2, date: "02/06" },
    { name: "Canon in D", level: 1, date: "02/06" },
    { name: "Minuet in G Major", level: 1, date: "02/05" },
    { name: "Ave Maria", level: 1, date: "02/05" },
    { name: "Arabesque No. 1", level: 1, date: "02/05" },
    { name: "River Flows in You", level: 1, date: "02/05" },
];
